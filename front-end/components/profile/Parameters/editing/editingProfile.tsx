...
export default function EditProfile(props:user){
    ...
    return ( <>
        { Version == 0 &&<div className="params__container">
          <PicEdit user={props.user} update={props.update} />
          <div className="options__container">
            <ProfileEdit user={props.user} callBack={setImg2fa} state={setVersion} update={props.update} />
            <Logout />
          </div>
        </div>}
        {Version == 1 && <Enable2FAPopUP qrcode={img2fa} state={setVersion} update={props.update} />}
        {Version == 2 && <Disable2FAPopUP state={setVersion} update={props.update} />}
        </>);
}